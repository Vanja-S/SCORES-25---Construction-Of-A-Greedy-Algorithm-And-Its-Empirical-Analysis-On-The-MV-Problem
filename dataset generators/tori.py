...
def get_torus_properties(G, n, m):
    ...
    assert max_degree == min_degree == 4, f"Torus should be 4-regular, got degrees {min_degree}-{max_degree}"
...
def is_valid_torus(G, expected_size=None, expected_n=None, expected_m=None):
    ...
    try:
        ...
        if expected_n is not None and expected_m is not None:
            expected_edges = 2 * expected_n * expected_m  # Each node has degree 4, so 2*n*m edges
            actual_edges = G.number_of_edges()
            if actual_edges != expected_edges:
                return False, f"Expected {expected_edges} edges for {expected_n}×{expected_m} torus, got {actual_edges}"
        ...
        if expected_n is not None and expected_m is not None:
            ...
            if actual_size != expected_n * expected_m:
                return False, f"Expected {expected_n * expected_m} nodes for {expected_n}×{expected_m} torus, got {actual_size}"
...
def create_torus_summary(torus_data, output_dir):
    ...
    mv_bounds = [data["properties"]["mutual_visibility_upper_bound"] for data in torus_data]
    ...
    summary = {
        ...
        "torus_dimensions": {
            "dimension_pairs": list(set((data["properties"]["torus_width"], data["properties"]["torus_height"]) for data in torus_data))
        }
    }
...
def generate_torus_dataset():
    ...
    for size_config in size_configs:
        ...
        for instance in range(num_instances):
            ...
            try:
                ...
                if not is_valid:
                    print(f"❌ Generated invalid torus for instance {instance}: {error_msg}")
                    continue
                ...
                print(f"  ✅ Generated torus {instance:03d}: {n}×{m} torus ({n*m} nodes) -> {filename}")
    ...
    if all_torus_data:
        ...
        overall_summary = {
            ...
            "size_breakdown": {
                config["label"]: len([d for d in all_torus_data if d["properties"]["nodes"] <= config["target"] * 1.5])
            ...
            "overall_statistics": {
                "nodes": {
                    ...
                    "mean": round(sum(p["nodes"] for p in all_properties) / len(all_properties), 2)
                },
                "mutual_visibility_upper_bounds": {
                    "min": min(p["mutual_visibility_upper_bound"] for p in all_properties),
                    "max": max(p["mutual_visibility_upper_bound"] for p in all_properties),
                    "mean": round(sum(p["mutual_visibility_upper_bound"] for p in all_properties) / len(all_properties), 2)
                }
            }
        }
    ...
    for size_label in size_categories:
        ...
        for entry in dataset_info:
            ...
            try:
                ...
                # Validate torus
                is_valid, error_msg = is_valid_torus(G, expected_nodes, expected_n, expected_m)
                
                if is_valid:
                    valid_tori += 1
                    print(f"  ✅ {filename}: Valid {expected_n}×{expected_m} torus ({expected_nodes} nodes)")
                else:
