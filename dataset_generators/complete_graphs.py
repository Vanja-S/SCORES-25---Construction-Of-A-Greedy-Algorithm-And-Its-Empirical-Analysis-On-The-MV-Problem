...
COMPLETE_GRAPH_SPECS = {
    "n10": {"nodes": 10, "instances": NUM_INSTANCES_PER_SET},
    "n100": {"nodes": 100, "instances": NUM_INSTANCES_PER_SET_N100},
}
...
def generate_complete_graph_dataset(base_dir, complete_specs):
    ...
    for size_cat, specs in complete_specs.items():
        ...
        complete_graph_dir = os.path.join(
            base_dir, size_cat, "complete"
        )  # New directory for complete graphs
        ...
        for i in range(num_instances):
            ...
            current_diameter = (
                1 if num_nodes > 1 else 0
            )  # Diameter of K_n is 1 (for n > 1)
            current_girth = (
                3 if num_nodes >= 3 else None
            )  # Girth of K_n is 3 (for n >= 3, undefined for K1, K2)

            # Average Shortest Path Length (needed for hypergraph Omega bound)
            avg_shortest_path_length = None
            if (
                num_nodes > 1
            ):  # Undefined for single node, or if graph is not strongly connected for directed
                try:
                    avg_shortest_path_length = nx.average_shortest_path_length(G)
                except (
                    nx.NetworkXError
                ):  # Can happen if graph not connected (not for K_n)
                    avg_shortest_path_length = None
    ...
    print(f"\nAll Complete graph generation complete.")
