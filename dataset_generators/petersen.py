...
def get_petersen_properties(G, n, k):
    ...
    if num_nodes <= 100: # Threshold for calculations like diameter, radius, center, ASPL
        if is_connected:
            try:
    ...
    properties = {
        ...
        "girth": girth,
        "is_planar": is_planar,
        "clustering_coefficient": nx.average_clustering(G), # Add remaining properties from original user code
        ...
        for n_petersen in valid_n_values:
            ...
            for k_petersen in valid_k_values:
                ...
                try:
                    ...
                    properties["seed"] = 10000 + graph_count # For consistency with other scripts
                    properties["filename"] = f"petersen_{graph_count:03d}_gp_{n_petersen}_{k_petersen}.gml"
                    ...
                    json_info = {k: v if v != "None" else None for k, v in properties.items()}
                    petersen_data.append(json_info)
                    
                    print(f"    ✅ Generated graph {graph_count:03d}: GP({n_petersen},{k_petersen}) ({2*n_petersen} nodes) -> {properties['filename']}")
                    graph_count += 1
                    
                except Exception as e:
                    print(f"    ❌ Failed to generate GP({n_petersen},{k_petersen}): {e}")
                    continue
        
        if petersen_data:
            # Calculate summary statistics
            node_counts = [g["nodes"] for g in petersen_data]
            diameters = [g["diameter"] for g in petersen_data if g["diameter"] != float('inf')]
