...
MYCIELSKIAN_GRAPH_SPECS = {
    "n10": {"nodes": 10, "instances": NUM_INSTANCES_PER_SET},  # Base graph size n_base
    "n100": {
...
def generate_mycielskian_dataset(base_dir, mycielskian_specs):
    ...
    for size_cat, specs in mycielskian_specs.items():
        ...
        base_graph_types_to_generate = ["path", "star_universal_vertex", "cycle"]

        for base_graph_name in base_graph_types_to_generate:
            print(
                f"  Generating M(G) for base_type='{base_graph_name}' (n={n_base})..."
            )
            ...
            for i in range(num_instances):
                ...
                if base_graph_name == "path":
                    ...
                    if n_base >= 2:
                        ...
                        print(
                            f"    Skipping star_universal_vertex for n_base={n_base} (too small)."
                        )
                        continue
                ...
                avg_degree_mycielskian = (
                    sum(dict(M_G.degree()).values()) / num_nodes_mycielskian
                    if num_nodes_mycielskian > 0
                    else 0
                )
                max_degree_mycielskian = (
                    max(dict(M_G.degree()).values()) if num_nodes_mycielskian > 0 else 0
                )
                ...
                if num_nodes_mycielskian <= 500:
                    ...
                    try:
                        ...
                        pass
                    try:
                        mycielskian_avg_shortest_path_length = (
                            nx.average_shortest_path_length(M_G)
                        )
                    except nx.NetworkXError:
                        pass
                else:
                    print(
                        f"    Skipping detailed property calculations for M(G) n={num_nodes_mycielskian}."
                    )

                # General Bounds for mu(G) (from Di Stefano et al. (2021))
                general_lb_max_degree_myc = max_degree_mycielskian

                general_ub_diameter_myc = (
                    num_nodes_mycielskian - mycielskian_diameter + 1
                    if mycielskian_diameter is not None
                    else None
                )
                ...
                if (
                    ...
                    hypergraph_omega_sqrt_n_D_lower_bound_val = math.sqrt(
                        num_nodes_mycielskian / mycielskian_avg_shortest_path_length
                    )
                ...
                M_G.graph["filename"] = (
                    f"{graph_id_counter:03d}_mycielskian_{base_graph_name}_{n_base}_{i}.gml"
                )
                ...
                M_G.graph["mutual_visibility_lower_bound"] = (
                    mutual_visibility_lower_bound
                )
                M_G.graph["mutual_visibility_upper_bound"] = (
                    mutual_visibility_upper_bound
                )
                ...
                M_G.graph["avg_shortest_path_length"] = (
                    mycielskian_avg_shortest_path_length
                )
                ...
                M_G.graph["hypergraph_omega_sqrt_n_D_lower_bound_val"] = (
                ...
                json_metadata = {
                    k: v if v != "None" else None for k, v in M_G.graph.items()
                }
        ...
        print(
            f"  Generated {len(dataset_info)} Mycielskian graphs for n={n_base} base graphs."
        )

    print(f"\nAll Mycielskian graph generation complete.")
