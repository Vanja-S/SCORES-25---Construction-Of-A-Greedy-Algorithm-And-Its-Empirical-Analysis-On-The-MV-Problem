...
def generate_connected_erdos_renyi_dataset(base_dir, graph_specs):
    ...
    for size_cat, specs in graph_specs.items():
        ...
        for p_val in p_values:
            ...
            for i in range(num_instances):
                ...
                if not is_connected:
                    print(f"    Warning: Could not generate connected G({n},{p_val}) after {max_attempts} attempts. Skipping instance {i}.")
                    continue
                ...
                if num_nodes <= 1000: # Threshold can be adjusted based on available computation power
                    try:
                ...
                if num_nodes <= 50: # Threshold can be adjusted
                    ...
                    except nx.NetworkXError: # Graph is acyclic (a tree) or issues finding cycles for sparse graphs
                        current_girth = None
                else:
                ...
                if num_nodes <= 1000: # Apply same threshold as diameter
                     try:
                         current_avg_shortest_path_length = nx.average_shortest_path_length(G)
                     except nx.NetworkXError: # Graph might be disconnected or paths not found
                         current_avg_shortest_path_length = None
                else:
                    print(f"    Skipping average shortest path length calculation for n={num_nodes} (potentially very slow).")
                ...
                if current_avg_shortest_path_length is not None and current_avg_shortest_path_length > 0:
                    hypergraph_omega_sqrt_n_D_lower_bound_val = math.sqrt(num_nodes / current_avg_shortest_path_length)
                ...
                G.graph['mutual_visibility_lower_bound'] = mutual_visibility_lower_bound # N/A for ER
                G.graph['mutual_visibility_upper_bound'] = mutual_visibility_upper_bound # N/A for ER
                ...
                G.graph['hypergraph_omega_sqrt_n_D_lower_bound_val'] = hypergraph_omega_sqrt_n_D_lower_bound_val 
                G.graph['graph_type'] = "erdos_renyi"
                G.graph['size_category'] = size_cat
                G.graph['instance'] = i
                G.graph['seed'] = G.graph.get('seed', None) # Get seed from NetworkX internal, or None
    ...
    # Save a final overall summary JSON for all ER graphs generated
    overall_summary_path = os.path.join(base_dir, "overall_erdos_renyi_summary.json")
    with open(overall_summary_path, 'w') as f:
        json.dump(overall_summary, f, indent=4)
    print(f"\nAll Erdős-Rényi graph generation complete. Total unique graphs generated: {len(overall_summary)}.")
