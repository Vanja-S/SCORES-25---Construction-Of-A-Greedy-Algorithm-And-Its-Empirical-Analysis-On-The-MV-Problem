...
def random_mv_sampling_size(G: nx.Graph, T: int = 10_000) -> Set[Any]:
    """
    Random sampling by first choosing a subset size k uniformly
    from 1..n, then sampling a k-subset of V(G).
    ...
    for _ in range(T):
        k = random.randint(1, n)  # random size
        S = set(random.sample(nodes, k))  # uniformly random k‐subset
        if len(S) > len(best_set) and mv(G, S):
            best_set = S
...
def ga_mutual_visibility(G, n_p=50, maxit=100, penalty_multiplier=10):
    ...
    for _ in range(maxit):
        pop_with_fitness = [
            (get_fitness(tuple(chromo)), chromo) for chromo in population
        ]
    ...
    final_fitness = get_fitness(
        tuple(np.array([1 if n in best_solution_ever else 0 for n in nodes]))
    )
...
def forward(G: nx.Graph, run_brute_force=True, draw_graph=False):
    ...
    if draw_graph:
        ...
        plt.title(
            f"{G.name if G.name else 'Graph'} - Mutual Visibility Set (Red Nodes)"
        )
